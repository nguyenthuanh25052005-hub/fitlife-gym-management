# 16. Team Code Distribution

## 1. Tổng quan phân công code

FitLife Gym Management System được chia theo module để 6 thành viên có phạm vi trách nhiệm rõ ràng, dễ phối hợp Git, dễ review code và dễ trình bày khi demo.

Mỗi thành viên phụ trách một phần backend, frontend, testing hoặc tài liệu liên quan đến module chính của mình.

## 2. Bảng phân công chính

| Thành viên | Vai trò | Backend code phụ trách | Frontend code phụ trách | Module chính |
|---|---|---|---|---|
| Thu Ánh | Backend Core & Database Developer | `backend/src/app.js`<br>`backend/src/server.js`<br>`backend/src/config/`<br>`backend/src/database/`<br>`backend/src/database/schema.sql`<br>`backend/src/database/seed.js` | Hỗ trợ kết nối API khi cần | Core backend, SQLite database, cấu trúc dự án |
...
| Member 5 | Trainer & Schedule Developer | `backend/src/routes/trainer.routes.js`<br>`backend/src/controllers/trainer.controller.js`<br>`backend/src/services/trainer.service.js`<br>`backend/src/routes/schedule.routes.js`<br>`backend/src/controllers/schedule.controller.js`<br>`backend/src/services/schedule.service.js` | `frontend/trainer-dashboard.html`<br>`frontend/js/trainer.js` | Trainer, lịch tập, kiểm tra trùng lịch |
| Member 6 | Frontend, Testing & Documentation Developer | `backend/tests/`<br>`.github/workflows/ci.yml`<br>`backend/.eslintrc` hoặc `eslint.config.js` | `frontend/css/style.css`<br>`frontend/js/api.js`<br>`frontend/admin-dashboard.html`<br>`frontend/js/admin.js` | Giao diện, test, CI/CD, tài liệu |
...
| Nội dung | Chi tiết |
|---|---|
| Backend files | `backend/src/app.js`, `backend/src/server.js`, `backend/src/config/database.js`, `backend/src/database/schema.sql`, `backend/src/database/seed.js` |
| Công việc chính | Khởi tạo backend Node.js + Express, cấu hình SQLite, tạo schema database, seed data, health check API |
...
| Nội dung | Chi tiết |
|---|---|
| Backend files | `backend/src/routes/auth.routes.js`, `backend/src/controllers/auth.controller.js`, `backend/src/services/auth.service.js`, `backend/src/middlewares/auth.middleware.js` |
| Frontend files | `frontend/login.html`, `frontend/register.html`, `frontend/js/auth.js` |
...
| Nội dung | Chi tiết |
|---|---|
| Backend files | `backend/src/routes/plan.routes.js`, `backend/src/controllers/plan.controller.js`, `backend/src/services/plan.service.js` |
| Frontend files | `frontend/index.html`, `frontend/js/plans.js` |
| Công việc chính | Xem danh sách gói tập, thêm gói tập, sửa gói tập, xóa mềm gói tập |
...
| Nội dung | Chi tiết |
|---|---|
| Backend files | `backend/src/routes/membership.routes.js`, `backend/src/controllers/membership.controller.js`, `backend/src/services/membership.service.js`, `backend/src/routes/payment.routes.js`, `backend/src/controllers/payment.controller.js` |
| Frontend files | `frontend/member-dashboard.html`, `frontend/js/member.js` |
...
| Nội dung | Chi tiết |
|---|---|
| Backend files | `backend/src/routes/trainer.routes.js`, `backend/src/controllers/trainer.controller.js`, `backend/src/services/trainer.service.js`, `backend/src/routes/schedule.routes.js`, `backend/src/controllers/schedule.controller.js`, `backend/src/services/schedule.service.js` |
| Frontend files | `frontend/trainer-dashboard.html`, `frontend/js/trainer.js` |
...
| Nội dung | Chi tiết |
|---|---|
| Frontend files | `frontend/css/style.css`, `frontend/js/api.js`, `frontend/admin-dashboard.html`, `frontend/js/admin.js` |
| Test/CI files | `backend/tests/`, `.github/workflows/ci.yml`, ESLint config |
| Docs files | `README.md`, `docs/` |
...
### Quy trình tạo branch, commit, push, pull request
...
Sau đó tạo Pull Request vào `main` để review và merge.
...
|---|---|
| Thu Ánh | Tổng quan backend, database và cấu trúc dự án |
| Member 2 | Đăng ký, đăng nhập, JWT và phân quyền |
| Member 3 | Quản lý gói tập |
| Member 4 | Đăng ký gói tập và payment mô phỏng |
| Member 5 | Đặt lịch trainer và cập nhật trạng thái lịch |
| Member 6 | Frontend, test, GitHub Actions và tài liệu |